...
- test-results.txt is now a tracked evidence artifact. The local pytest
  snapshot is regenerated by `scripts/refresh-test-results.ps1`, which
  normalizes timing, platform, and pytest-version lines so identical
  evidence produces a byte-identical file. CI regenerates the snapshot
  and fails any push that produces drift, so the committed numbers
