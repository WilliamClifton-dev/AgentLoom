...
- Current public-main gate: `379 passed / 0 skipped` with Docker tests
  enabled.
- Local Lite gate: `376 passed / 3 skipped / 0 failed`. The 3 skips are
  `tests/test_docker_sandbox_live.py`; with a live Docker daemon the
  same three tests pass, taking the count to 379/0/0.
...
- `test-results.txt` is tracked in git and byte-stable; the CI workflow
  runs `scripts/refresh-test-results.ps1` and fails the PR on drift.

The remaining Human checkpoints are competition-page submission, a
second-host Full bootstrap reproduction, and a third-party signature
on the submission package (P3 follow-up; the operator runbook and
the cosign wrapper are committed but the actual run is pending).

## What changed since the 2026-08-16 handoff

### P0: evidence-baseline alignment

- `test-results.txt` is now tracked, regenerated by
  `scripts/refresh-test-results.ps1`, and gated by a new
  `Refresh and verify test-results.txt` step in the CI workflow
  (`.github/workflows/ci.yml`).
- The PowerShell `if (git diff --exit-code ...)` form was inverted by
  the platform's 0-is-false semantics; the gate now reads
  `$LASTEXITCODE` explicitly so a drift PR fails for the right
  reason.
...
### P3: second-host bootstrap + cosign signing (runbooks landed)
...
- `docs/security/signing-submission.md` documents the signing and
  verification flow.
...
The actual second-host run and the actual cosign signature are
pending operator action; the local environment cannot install
cosign (winget cannot reach the Sigstore source). The scripts
themselves are runnable on any host that has `cosign` on `PATH`.
...
- Prefer deterministic replay for diagnostics. Every new live run
  ...
  model.
- Do not claim PR #1141 is merged, second-host Full reproduction
  is complete, cosign signing has run, or competition submission
  is complete without direct external proof (the GitHub release
  page, the Rekor entry URL, or the second-host compatibility
  record).
...
a third-party signature on the submission package, follow
