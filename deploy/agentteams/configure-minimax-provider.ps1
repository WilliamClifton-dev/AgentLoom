...
$results = foreach ($container in $Containers) {
    ...
    $providers = Invoke-CoPaw -BaseUri $baseUri -Method Get `
        -Path "/api/models" -Body $null
    $minimax = $providers | Where-Object { $_.id -eq $ProviderId }
    if ($null -eq $minimax) {
    ...
    $minimax = $providers | Where-Object { $_.id -eq $ProviderId }
    if ($null -eq $minimax) {
        throw "MiniMax provider is unavailable in $container"
    }
    $modelIds = (@($minimax.models) + @($minimax.extra_models)) |
        ForEach-Object { $_.id }
    ...
    if (-not $SkipConnectionTest) {
        ...
        if (-not $probe.success) {
            throw "MiniMax model probe failed in $container"
        }
