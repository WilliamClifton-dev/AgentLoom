<#
.SYNOPSIS
    Sign the public submission package with sigstore cosign.

.DESCRIPTION
    Implements the P3 "third-party digital signature" line of the active
    goal. The script wraps `cosign sign-blob` to produce a detached
    signature for the public submission ZIP referenced in
    `docs/competition/submission-package-manifest.json`. The signature
    is published alongside the package in `artifacts/signatures/` and
    the verification flow is documented in
    `docs/security/signing-submission.md`.

    The script is designed to be run on the second host after the
    public-main gate has produced a clean submission package. It
    fails closed if any of the following is missing: cosign, the
    submission ZIP, or a non-empty AGENTLOOM_COSIGN_KEY (a path to a
    cosign-generated `cosign.key`, or the literal `keyless://` to
    use Sigstore's keyless flow against the public Fulcio CA).

.PARAMETER PackagePath
    Path to the submission ZIP. Defaults to the manifest's package file
...
.PARAMETER CosignBin
    Path to the cosign binary. Defaults to `cosign` on PATH; set this
    when cosign lives outside PATH (e.g. user-local install).
...
.EXAMPLE
    # Keyless signing with Sigstore Fulcio + Rekor (recommended for public
    # competition submissions where the private key is ephemeral):
...
AGENTLOOM_COSIGN_KEY is not set. Set it to either:
  ...
  - the literal string 'keyless://' to sign with the Sigstore Fulcio CA
    (recommended for public competition submissions).
"@
}
...
# Verify the signature was produced and is non-empty. A zero-byte
# signature would still let `cosign verify-blob` pass in some
# misconfigurations, so we require a real payload here.
